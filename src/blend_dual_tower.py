"""
Blend Dual-Tower predictions with public notebook submissions.
"""
...
def blend_submissions(dual_tower_weight: float = 0.3):
    """Blend Dual-Tower with SOTA+ESM2 average."""
    print(f"\n=== Blend Dual-Tower (weight={dual_tower_weight}) ===\n")

    # Load all submissions
    dual_tower = load_submission(DUAL_TOWER_PATH, "Dual-Tower")
    sota = load_submission(SOTA_PATH, "SOTA 27Jan")
    esm2 = load_submission(ESM2_PATH, "ESM2 3785")

    # Get Dual-Tower GO terms
    dt_terms = set()
    for protein_scores in dual_tower.values():
        dt_terms.update(protein_scores.keys())
    print(f"\nDual-Tower covers {len(dt_terms)} GO terms")

    # Get all proteins
    all_proteins = set(dual_tower.keys()) | set(sota.keys()) | set(esm2.keys())
    ...
    with open(output_path, 'w') as f:
        for i, protein in enumerate(sorted(all_proteins)):
            ...
            all_terms = set()
            if protein in dual_tower:
                all_terms.update(dual_tower[protein].keys())
            if protein in sota:
            ...
            for term in all_terms:
                ...
                dt_score = dual_tower.get(protein, {}).get(term, 0)
                ...
                # Blend strategy
                if dt_score > 0 and baseline > 0:
                    score = dt_score * dual_tower_weight + baseline * (1 - dual_tower_weight)
                elif dt_score > 0:
                    score = dt_score
                else:
                    score = baseline
