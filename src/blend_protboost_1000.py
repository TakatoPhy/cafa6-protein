"""
Blend ProtBoost 1000 predictions with existing best submission.
Run on remote server after protboost_1000.tsv is created.
"""
...
# Input files
PROTBOOST_PATH = SUBMISSIONS_DIR / 'protboost_1000.tsv'
...
def blend_submissions(protboost_weight: float = 0.3):
    """Blend ProtBoost 1000 with SOTA+ESM2 average."""
    print(f"\n=== Blend ProtBoost 1000 (weight={protboost_weight}) ===\n")

    # Load all submissions
    protboost = load_submission(PROTBOOST_PATH, "ProtBoost 1000")
    ...
    print(f"\nProtBoost 1000 covers {len(protboost_terms)} GO terms")
    ...
    # Blend
    output_path = SUBMISSIONS_DIR / f'blend_protboost1000_{int(protboost_weight*100)}.tsv'
