"""
Simple ProtBoost: LightGBM model for GO term prediction using multiple embeddings.
Inspired by CAFA5 2nd place solution.

Strategy:
1. Use ESM2-650M embeddings (already have)
2. Train LightGBM for top GO terms
3. Generate predictions and blend with public notebook scores
"""
import numpy as np
import pandas as pd
...
# Configuration
N_TOP_TERMS = 500  # Number of GO terms to train on
PCA_DIM = 256      # PCA dimension for embeddings
SEED = 42
...
def load_embeddings(name: str):
    ...
    # Extract UniProt IDs (e.g., "sp|A0A0C5B5G6|MOTSC_HUMAN" -> "A0A0C5B5G6")
...
def main():
    print("=== ProtBoost Simple ===\n")

    # Load ESM2-650M embeddings
    ...
    # Create label matrix for training proteins
    ...
    # Also transform test embeddings
    ...
    for i, (term, model) in enumerate(zip(top_terms, models)):
        ...
        if model is None:
            # Use mean training frequency as fallback
    ...
    test_predictions = np.array(test_predictions).T  # (n_test, n_terms)
    ...
    output_path = OUTPUT_DIR / 'protboost_simple.tsv'
